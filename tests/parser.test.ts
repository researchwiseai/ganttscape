...
`;
    ...
    it('throws when parent label does not exist', () => {
        const bad = [{ label: 'A', start: '2021-01-01', end: '2021-01-02', parent: 'B' }];
        const file = writeTempFile(JSON.stringify(bad), '.json');
    ...
    it('parses dates with trailing Z after offset', () => {
        ...
        const file = writeTempFile(JSON.stringify(tasks), '.json');
        const schedule = parseSchedule(file);
        expect(schedule.tasks[0].start.toISOString()).toBe('2025-05-20T12:00:01.967Z');
        expect(schedule.tasks[0].end.toISOString()).toBe('2025-05-20T12:00:03.335Z');
        rmSync(file, { recursive: true, force: true });
    });
